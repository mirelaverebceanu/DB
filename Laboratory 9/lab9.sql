...
AS
SELECT * FROM ex5_1_lab9('Sisteme de operare')
EXEC ex5_lab9;

DROP PROCEDURE dbo.ex5_lab9
SELECT * FROM studenti.studenti_reusita 
GO
DROP FUNCTION dbo.ex5_1_lab9
GO 
CREATE FUNCTION ex5_1_lab9(@disciplina varchar (100))
RETURNS TABLE
AS
RETURN
(SELECT DISTINCT TOP(3) Cod_Grupa
		...
		,Disciplina
		,Nota as nota_veche
		,case when Nota=10 then Nota
			  else Nota+1 
		  end as nota_noua
FROM studenti.studenti_reusita
JOIN grupe ON studenti_reusita.Id_Grupa = grupe.Id_Grupa
